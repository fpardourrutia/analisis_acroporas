...
validacion_cruzada_por_segmentos <- function(df, nombre_variable_segmentacion, formula_prediccion, num_iteraciones){
  ...
  # Función que permite calcular el error de validación cruzada al predecir sobre
  # un segmento (fold) de los datos, entrenando sobre los anteriores.
  # Segmento es un string con el valor de la variable de segmentación sobre el que
  # se realizará la predicción, los otros datos serán de entrenamiento
  # La función regresa un df con el nombre del segmento, el número de elementos en
  # el segmento y el error de entrenamiento correspondiente.
  calcula_error_segmento <- function(segmento, df, nombre_variable_segmentacion, formula_prediccion){
    ...
    # Obteniendo el error de predicción para el segmento
    error_segmento <- mean(predicciones_prueba != set_prueba[[y]])
    
    # Formando el resultado
    resultado <- data_frame(
      segmento = segmento,
      n = nrow(set_prueba),
      error_segmento = error_segmento
    )
  ...
  errores <- ldply(1:num_iteraciones, function(i, df, nombre_variable_segmentacion, formula_prediccion, .parallel = TRUE){
    ...
    #  Llamando la función "calcula_error_segmento" para todos los segmentos en el df.
    errores_segmentos <- ldply(segmentos, calcula_error_segmento,
      df, nombre_variable_segmentacion, formula_prediccion)
...
exito_vc_cat_latitud_4 <- weighted.mean(cv_cat_latitud_4$exito_segmento,
  cv_cat_latitud_4$n) #0.519145

# Calculando las proporciónes de éxito por iteración, para graficarlos al
# final contra una simulación de un clasificador aleatorio
exito_iteracion_cat_latitud_4 <- cv_cat_latitud_4 %>%
  group_by(num_iteracion) %>%
  summarise(
    exito = weighted.mean(exito_segmento, n)
  )

# Graficando las proporciones de exito por fold:
ggplot(cv_cat_latitud_4,
  aes(x = exito_segmento, fill = segmento, group = segmento)) +
...
# Tabla con número de puntos por categorñia de latitud y arrecife
df_trabajo_2010_variables_selectas %>%
...
# Calculando la proporción de éxito de VC:
exito_vc_cat_latitud_5 <- weighted.mean(cv_cat_latitud_5$exito_segmento,
  cv_cat_latitud_5$n)
...
# Al calcular 4 vs 5 folds, podemos ver cómo la predicción para Limones y Manchones aumenta
# considerablemente al pasar de 4 a 5, los efectos pueden deberse a distintas causas:
# Para limones:
# 1. Es posible que como la 4-CV, cuando tenía que entrenarpara predecir sobre el fold correspondiente a Limones, tenía una cantidad de datos insuficiente de datos de entrenamiento
# (36 datos). Esto se sugiere con el hecho de que en los análisis exploratorios de variables
# satelitales por arrecife, Limones, para algunas variables, alcanzaba rangos que los otros
# arrecifes no. En esta línea, es de notar que para el fold 5, Limones queda mucho mejor
# representado tanto en datos de entrenamiento como de prueba, y esto mejora la predicción sobre # éste.
# 3. Sin embargo, algo que hay que notar es que al pasar de 4-CV a 5-CV5, la predicción sobre
# Limones pasa a ser de la peor a la mejor, lo que sugiere que, aún controlando por
# autocorrelación espacial sobre éste (puesto que con su parte "Sur" estamos prediciendo su
# parte "Norte"), mejora mucho la predicción, lo que abre la pregunta de investigar si
# existen "agregados" con distintas relaciones entre las variables satelitales y cobertura
# de acrópora, y si convendría más realizar el análisis con estos agregados.
