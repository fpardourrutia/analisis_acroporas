...
# Para poner varios plots en un mismo recuadro, usando "plot_grid", y
# conservando colores de los factores
#library("cowplot")
...
# Para calcular importancia de variables, que al final no da tiempo de programar
library("sperrorest")
...
ggplot(data = df_plot_2010,
  aes(x = cat_latitud_4, y = valor)) +
  geom_boxplot() + 
  facet_wrap(~llave, scales = "free")
ggplot(data = df_plot_2010,
  aes(x = cat_latitud_5, y = valor)) +
  geom_boxplot() + 
  facet_wrap(~llave, scales = "free")
ggplot(data = df_plot_2010,
  aes(x = cat_latitud_10, y = valor)) +
  geom_boxplot() + 
  facet_wrap(~llave, scales = "free")
```
...
cat_latitud_4_folds_plot <- ggplot(df_trabajo_2010_variables_selectas,
  aes(
    x = longitud,
    y = latitud,
  ...
  geom_point() +
  labs(title = "4-Validación Cruzada", colour = "Segmento") +
  theme(
    plot.title = element_text(hjust = 0.5, face="bold")
  )

cat_latitud_5_folds_plot <- ggplot(df_trabajo_2010_variables_selectas,
  aes(
    x = longitud,
    y = latitud,
  ...
  geom_point() +
  labs(title = "5-Validación Cruzada", colour = "Segmento") +
  theme(
    plot.title = element_text(hjust = 0.5, face="bold")
  )

cat_latitud_10_folds_plot <- ggplot(df_trabajo_2010_variables_selectas,
  aes(
    x = longitud,
    y = latitud,
  ...
  geom_point() +
  labs(title = "10-Validación Cruzada", colour = "Segmento") +
  theme(
    plot.title = element_text(hjust = 0.5, face="bold")
  )

# plot_grid(cat_latitud_4_folds_plot,
#   cat_latitud_5_folds_plot,
#   cat_latitud_10_folds_plot,
#   ncol = 3, nrow = 1)
...
```{r, warning=FALSE, message=FALSE}
# Calculando las proporciones de éxito por iteración, para graficarlas 
# contra una simulación de un clasificador aleatorio
exito_iteracion_numero_folds <- cv_cat_latitud_4 %>%
  ...
  # quiero tener los factores para la tabla de p-values
  mutate(
    tipo = factor(tipo, levels = c(
      "RF variables satelitales 4-fold CV",
      "RF variables satelitales 5-fold CV",
      "RF variables satelitales 10-fold CV"
    ))
  ) %>%
  select(
    tipo,
...
# Calculando las medias de éxito de iteración por número de folds:
medias_exito_iteracion_numero_folds <- exito_iteracion_numero_folds %>%
  group_by(tipo) %>%
  summarise(
    porcentaje_exito_promedio = mean(porcentaje_exito)
  )
...
ggplot(plot_distribuciones_aleatorio_rf,
  ...
  geom_vline(
    xintercept = medias_exito_iteracion_numero_folds$porcentaje_exito_promedio,
    linetype = "dotted") +
  geom_vline(
    xintercept = 0.5,
    linetype = "dotted") +
  xlab("porcentaje de éxito") +
  ylab("n") +
  ggtitle("Histogramas de éxito de predicción") +
  theme(
    plot.title = element_text(hjust = 0.5, face="bold")
  )
...
ddply(medias_exito_iteracion_numero_folds, .(tipo), function(x, simulacion_error_azar){
  p_value <- simulacion_error_azar %>%
  filter_(paste0("porcentaje_exito >= ", x$porcentaje_exito_promedio)) %>%
  summarise(
  ...
  resultado <- data_frame(
    tipo = x$tipo,
    media = x$porcentaje_exito_promedio,
    p_value = p_value
  )
}, simulacion_error_azar) %>%
  mutate(
    tipo = ifelse(tipo == "RF variables satelitales 4-fold CV", "VC 4 segmentos",
      ifelse(tipo == "RF variables satelitales 5-fold CV", "VC 5 segmentos",
        "VC 10 segmentos")
    )
  ) %>%
  View()
...
4-folds: a menor poder predictivo, más ruido en variables importantes.
10-folds: el efecto de autocorrelación espacial (en la variable de salida) podría
sesgar la importancia de ciertas variables.
Un buen tradeoff por lo pronto es usar k=5, una elección más cuidadosa involucraría
el uso de un correlograma.
...
summary(cv_k_folds_cat_latitud_5_importancia_variables$importance) %>%
  ...
  transmute(
    variable = variables,
    incremento_error_promedio = round(mean.error, 4)
  ) %>%
  View()
```
